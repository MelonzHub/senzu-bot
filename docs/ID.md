# Dokumentasi Senzu Bot (ID)
Dokumen ini menjelaskan setup, konfigurasi, dan pengembangan plugin untuk Senzu Bot.
...
```js
global.creator = 'Melonz Velia'
global.Api = AlyaApi(process.env.API_ENDPOINT, process.env.API_KEY)
global.header = `Senzu Bot v${require('package.json').version}`
```
...
## Struktur Plugin
Senzu Bot mendukung dua tipe plugin: command dan event.
